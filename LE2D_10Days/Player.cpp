...
void Player::Move(int windowLeft, int windowRight, int windowTop, int windowBottom, const char* keys) {
    if (keys[DIK_W]) pos.y -= speed.y;
    if (keys[DIK_S]) pos.y += speed.y;
    if (keys[DIK_A]) pos.x -= speed.x;
    if (keys[DIK_D]) pos.x += speed.x;
    ...
    if (pos.y + radius > windowBottom) pos.y = windowBottom - radius;
}
